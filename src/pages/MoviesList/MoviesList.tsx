...
const MoviesList: FC = () => {
  ...
  const { moviesDownloaded, moviesFiltered, loadingStateMovies } =
  ...
  const pageChangeInit = () => {
    if (
      ...
      chosenPageMovie !== paginationFiltered.total_pages
    ) {
  ...
  useEffect(() => {
    ...
    switch (true) {
      case !moviesFiltered.length &&
        !moviesDownloaded.length &&
        !searchNameMovie:
      ...
      case searchNameMovie && languageRef.current !== language:
        ...
        break;
      case JSON.stringify(chosenGenresMoviesIdRef.current) !==
        JSON.stringify(chosenGenresMoviesId):
  ...
  useEffect(() => {
    ...
    if (!!searchNameMovie) {
      ...
      dispatch(
        PaginationMovieAction.setPaginationFiltered(paginationDownloaded)
      );
    }
  }, [moviesDownloaded]);
