...
const MoviesList = () => {
  ...
  const { moviesFiltered, loadingStateMovies } = useAppSelector((state) => state.Movies);
  const { searchNameMovie, chosenGenresMoviesId, loadingStateGenres } = useAppSelector((state) => state.Search);
  const { chosenPageMovie } = useAppSelector((state) => state.ChosenPage);
  const { paginationFiltered, observer_position, scroll_position } = useAppSelector((state) => state.PaginationMovies);
  ...
  const pageChangeInit = () => {
    if (
      ...
      movieListContainerRef.current.scrollTop > movieListContainerRef.current.scrollHeight - 1500
    ) {
    ...
    dispatch(
      MoviesActions.searchMovies({
        ...
        query: `?page=${chosenPageMovie}&${
          searchNameMovie ? `query=${searchNameMovie}` : `with_genres=${chosenGenresMoviesId.join()}`
        }&language=${language}`,
      }),
    );
    ...
    <div className={styles.moviesListBase} onClick={closeSearchPanel}>
      ...
      {(loadingStateMovies || loadingStateGenres) && !moviesFiltered.length ? (
        ...
        <div className={styles.movieListContainer}>
          {moviesFiltered.length ? (
            moviesFiltered.map((movie, index) => <MovieCard key={index} movie={movie} />)
          ) : (
