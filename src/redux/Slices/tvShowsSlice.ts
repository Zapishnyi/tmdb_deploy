...
const TVShowsDownload = async ({
    ...
    const TVShowsRecursion = async (query: string, searchByTitle: boolean, TVShows: ITVShowsPaginated): Promise<ITVShowsPaginated> => {
        ...
        if (tempTVShows.total_pages !== tempTVShows.page && tempTVShows.page <= 39) {
            ...
            TVShows = await TVShowsRecursion(
                query.replace(/(?<=page=)\d*/, (++tempTVShows.page).toString()),
                searchByTitle,
                {...TVShows})
...
const searchTVShows = createAsyncThunk(
...
export const tvShowsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addMatcher(
                isRejected(searchTVShows, endlessPaginationAction),
                (state, action) => {
                    console.log(
                        "TVShows receive sequence failed with error:",
                        action.payload,
                    );
                },
            )
            .addMatcher(
                isPending(searchTVShows),
                (state) => {
